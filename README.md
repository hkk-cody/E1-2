...
## 회고
```plaintext
python을 이전에도 몇번 해보기도 했고 공부했었지만 경험이 많지도 않고, 매번 생소한 느낌이었는데 이번 기회에 다시 문법에 대해서 복습을 할 수 있었던 기회였습니다.
공부하면서 제너레이터라는 문법을 알게 됐는데 파일입출력에서 큰 파일인 경우에 보통 chunk단위로 쪼개서 읽거나 처리하는 경우가 많은데 이걸 언어에서 보다 쉽게 사용할 수 있게 지원한다는 점이 흥미로웠습니다.
```